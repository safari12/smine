...
class Datacenter {
  ...
  fetchRigs() {
    ...
    for (let i = 1; i <= this.numberOfRigs; i++) {
      ...
      const maxAttempts = this.api.max_attempts ? this.api.max_attempts : 3
      const retryDelay = this.api.retry_delay ? this.api.retry_delay : 3000
        ...
        request.get(uri, {
          ...
          .then((body) => {
            return new Rig({
              name: name,
              miner: 'xmr-stak',
              hashrate: _.last(body.hashrate.total),
              data: body,
            })
          })
          .catch(() => {
