...
class Datacenter {
  ...
  async getStats() {
    ...
    const fetchedOfflineRigs = this.getFetchedOfflineRigs(
      this.stats.rigs.seen,
      fetchedOnlineRigs
    );

    this.stats.rigs.new.online = this.getRigsThatBecameOnline(
      fetchedOnlineRigs,
      this.stats.rigs.seen
    );
    this.stats.rigs.new.offline = this.getRigsThatBecameOffline(
      fetchedOfflineRigs,
      this.stats.rigs.currently.offline
    );
    this.stats.rigs.fixed = this.getRigsThatAreFixed(
      this.stats.rigs.currently.offline,
      fetchedOfflineRigs
    );
        ...
        request
          ...
          .then(body => {
            return new Rig({
              name: name,
              miner: 'xmr-stak',
              hashrate: _.first(body.hashrate.total),
              data: body
            });
          })
          .catch(() => {
