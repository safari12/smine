...
class Notifications {
  ...
  notifyStatsIfNeeded(stats) {
    ...
    let fixedRigContent = this.getEmailContentIfAnyRigsChanged(
      stats.offlineRigs,
    ...
    if (fixedRigContent.length > 0 && stats.offlineRigs.length > 0) {
      fixedRigContent += html.generateHeaderTag(3, 'The following rigs are still offline')
      fixedRigContent += html.generateListTag(
